...
function verifyTOTP(secretB32, token, { period = 30, digits = 6, window = 1, time = Date.now() } = {}) {
  ...
  const counter = Math.floor(time / 1000 / period);
  const candidate = Buffer.from(clean, 'ascii');
  for (let i = -window; i <= window; i++) {
    const expected = Buffer.from(generateTokenFromCounter(secretB32, counter + i, digits), 'ascii');
    // Comparación en tiempo constante (misma longitud fija de `digits`).
