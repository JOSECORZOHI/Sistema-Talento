...
function scrubText(text, personalValues) {
  ...
  for (const v of personalValues) {
    ...
    const seg = stripAccentsAndLower(v)
      .split(/(?=[0-9])|(?<=[0-9])/)
      .map(escapeRe)
      .join('[.\\- ]?');
    if (stripAccentsAndLower(out).includes(stripAccentsAndLower(v))) {
...
async function replaceManyText(collection, personalValues, replacement = SCRUB_PATTERN) {
  ...
  // Se pagina con skip+limit para recorrer TODOS los documentos. Las
  // actualizaciones ($set sobre details/action/scrubbed) no alteran el orden ni
  // el filtro, por lo que el desplazamiento por ventanas es estable.
  let offset = 0;
  while (true) {
    const batch = await collection.find({}).skip(offset).limit(BATCH).toArray();
