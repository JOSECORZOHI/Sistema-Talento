 ...
 * @param {number} [options.maxPage=100000] - Tope máximo aceptado para `page` (evita `skip` desmesurados).
...
function parsePagination(req, options = {}) {
  ...
  const maxPage = Number.isInteger(options.maxPage) ? options.maxPage : 100000;
  const query = (req && req.query) || {};
  const rawPage = parseInt(query.page, 10);
  const page = Number.isInteger(rawPage) && rawPage > 0 ? Math.min(rawPage, maxPage) : 1;
